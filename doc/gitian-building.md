...
*Setup instructions for a Gitian build of Civilbit using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Civilbit
...
- [Building Civilbit](#building-civilbit)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-civilbit-host/README.md).
...
Clone the git repositories for civilbit and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/civilbit/civilbit
```
...
in the civilbit repository under 'Fetch and build inputs' to install sources which require
...
To build Civilbit (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the civilbit repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/civilbit/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/civilbit/civilbit
...
For example:
```bash
URL=https://github.com/laanwj/civilbit.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit civilbit=${COMMIT} --url civilbit=${URL} ../civilbit/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit civilbit=${COMMIT} --url civilbit=${URL} ../civilbit/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit civilbit=${COMMIT} --url civilbit=${URL} ../civilbit/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the civilbit git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../civilbit/contrib/gitian-descriptors/*|sort|uniq )
...
MBTPATH=/some/root/path/civilbit.git
SIGPATH=/some/root/path/civilbit-detached-sigs.git

./bin/gbuild --url civilbit=${MBTPATH},signature=${SIGPATH} ../civilbit/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/civilbit-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/civilbit-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/civilbit-osx-build.assert
```
...
[civilbit/gitian.sigs](https://github.com/civilbit/gitian.sigs/) repository, or if that's not possible create a pull
