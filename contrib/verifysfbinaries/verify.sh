...
###   This script attempts to download the signature file SHA256SUMS.asc from civilbit.org
...
WORKINGDIR="/tmp/civilbit"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://civilbit.org/bin/"
VERSIONPREFIX="civilbit-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'civilbit-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Civilbit Core binary release signing key installed?"
   fi
